...
class PaginaSemestre(BaseWindow):
    ...
    def __init__(self, semestre, conexao, service):
        ...
        super().__init__(conexao, title=f"Semestre {semestre.nome}", service=service)

    def _create_body(self) -> None:
        sem_frame = DisciplinasFrame(self.conexao, semestre=self.semestre, service=self.service, master=self)
